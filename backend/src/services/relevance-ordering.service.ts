...
import { RelevanceOrderingConfig } from '../config/thresholds.config';
...
export class RelevanceOrderingService {
  ...
  private static calculateDocumentQualityScore(context: DocumentContext): number {
    try {
      ...
      return RelevanceOrderingConfig.defaultQualityScore;
    }
  }
  ...
  private static calculateWebAuthorityScore(url: string): number {
    try {
      ...
      return RelevanceOrderingConfig.defaultAuthorityScore;
    }
  }
  ...
  private static calculateWebFreshnessScore(publishedDate?: string): number {
    if (!publishedDate) {
      return RelevanceOrderingConfig.defaultFreshnessScore;
    }
    ...
    try {
      ...
      if (daysDiff <= RelevanceOrderingConfig.freshnessTiers.veryRecent.days) {
        return RelevanceOrderingConfig.freshnessTiers.veryRecent.score;
      } else if (daysDiff <= RelevanceOrderingConfig.freshnessTiers.recent.days) {
        return RelevanceOrderingConfig.freshnessTiers.recent.score;
      } else if (daysDiff <= RelevanceOrderingConfig.freshnessTiers.moderate.days) {
        return RelevanceOrderingConfig.freshnessTiers.moderate.score;
      } else if (daysDiff <= RelevanceOrderingConfig.freshnessTiers.withinYear.days) {
        return RelevanceOrderingConfig.freshnessTiers.withinYear.score;
      } else {
        // Older: decay
        const decay = Math.max(RelevanceOrderingConfig.freshnessTiers.minDecayFactor, 1.0 - (daysDiff - 365) / 365);
        return decay;
      }
    } catch (e) {
      return RelevanceOrderingConfig.defaultFreshnessScore;
  ...
  static quickOrder(context: RAGContext): OrderedRAGContext {
    ...
    const orderedWebResults = [...context.webSearchResults]
      .map(r => ({ ...r, orderingScore: RelevanceOrderingConfig.defaultOrderingScore }))
      .sort((a, b) => (b.orderingScore || 0) - (a.orderingScore || 0));
